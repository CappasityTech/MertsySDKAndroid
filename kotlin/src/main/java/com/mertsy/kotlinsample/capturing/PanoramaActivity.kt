...
class PanoramaActivity : AppCompatActivity(R.layout.activity_panorama) {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        with(findViewById<MertsyCapturingView>(R.id.panorama)) {
            setBackPressedDispatcher(onBackPressedDispatcher)
            setFragmentManager(supportFragmentManager)
            setLifecycleOwner(this@PanoramaActivity)
